...
@st.cache_data
def load_articles(match_df: pd.DataFrame):
    df = match_df.copy()

    df["published"] = pd.to_datetime(
        df["art_publishedAt"], errors="coerce", utc=True
    )

    df["text"] = (
        "Title: " + df["art_title_en"].fillna("") +
        "\nSource: " + df["art_source_name"].fillna("") +
        "\n\n" + df["art_description_en"].fillna("")
    )
    ...
    ].dropna(subset=["published", "text"])

@st.cache_resource
def build_article_index(article_df: pd.DataFrame):
...
def retrieve_articles(
    ...
    if country:
        df = df[df["art_article_country"] == country]
    ...
    distances, indices = index.search(q_emb, min(k, len(df)))

    return df.iloc[indices[0]].to_dict(orient="records")

def get_conflict_context(conf_df: pd.DataFrame, country: str | None):
    if country:
        df = conf_df[conf_df["country"] == country]
    else:
        df = conf_df
    ...
    return df[existing].head(3).to_string(index=False)

def build_prompt(question, conflict_context, articles):
    article_block = "\n\n".join(
        [
            f"{a['art_title_en']} ({a['published'].date()})\n{a['text'][:1200]}"
            for a in articles
        ]
    )

    return f"""
You are a conflict analysis assistant.
